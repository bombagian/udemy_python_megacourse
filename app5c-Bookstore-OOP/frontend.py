...
def delete_command():
	database.delete(selected_tuple[0])#retrieve the value from the global variable.
	view_command()


def update_command():
	database.update(selected_tuple[0],title_txt.get(),author_txt.get(),year_txt.get(),isbn_txt.get()) #keep ID of selected row and send the changed values
	view_command()
